...
!	3.跳转到head模块执行
!
! 注意：因为时间问题，此启动代码不检测磁盘类型，而默认使用1.44M软盘。1.44M软盘有2个面（磁头号0和1），
! 每面80个磁道（磁道号0 ~ 79），每道18个扇区（1 ~ 18，没错，是1 ~ 18）。2 * 80 * 18 * 512B = 1440KB，
! 并没有1.44MB。
! **************************************************************************************************

! 设置以下值时，注意：内存0x10000以前为中断服务程序、中断表、BIOS数据区，内存640kb 以后为BIOS和显存区
! 域（ROM），这些区域起初均不能被占用。
BOOTSEG		= 0x07c0				! 启动代码被载入的位置
INITSEG		= 0x9000				! 启动代码被移动到的位置
SYSSEG		= 0x1000				! 系统被载入位置
TEMPSTK		= 0x9020				! 用于临时堆栈
SYSLEN		= 384					! 内核占用磁盘最大扇区数
...
! 加载内核代码到 SYSSEG:0，共 SYSLEN 个扇区。加载时，须防止"DMA超过64kb界限"这种错误发生。
! INT 13h 的使用方法如下：
...
canread:.word 128					! 为防止DMA超过64kb界限，最大可读取扇区数。该变量递减，若为0，则已读取128扇区（64kb），就重新从128开始。
...
! -----------------------------------------------
! 提示信息定义。msg1为载入提示信息，共 21 个字符；msg2为载入失败提示信息，共11个字符
msg1:
	.byte 13,10
...
gdt_48:
	.word 0x07ff					! 段限长
	.word gdt,9						! 注意!!!段基址不再是 0x7c00,0。嗯，花了半天功夫才发现 :-(
